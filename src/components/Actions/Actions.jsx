...
const Actions = () => {
  ...
  const slidesPreview = breakpoints.reduce(
    (slides, { max, slides: slidesForBreakpoint }) =>
      useMediaQuery({ query: `(max-width: ${max}px)` })
        ? slidesForBreakpoint
        : slides,
    actions.length
  );
  ...
  return (
    <>
      <div className='w-full pt-3 pl-4 pr-0 mt-10 border-2 h-36 rounded-t-2xl border-l-lime-500 border-r-lime-500 border-t-lime-500 bg-gradient-to-r from-zinc-100 via-gray-100 to-stone-200 lg:h-40 xl:h-32'>
        <div className='w-full h-full'>
          <Swiper
            ...
            {actions.map(({ id, name, icon }, index) => (
              <SwiperSlide key={id}>
                {/*  Container */}
                <div
